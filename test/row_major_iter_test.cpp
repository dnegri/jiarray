...
#define m_phif(k, l, ig) d_phif[(k * nxy + l) * ng + ig]
#define m_xsnf(k, l, ig) d_xsnf[(k * nxy + l) * ng + ig]
#define m_psi(k, l)      d_psi[k * nxy + l]
...
int iter_test() {
    ...
    Array<float, 3> phif(nz, nxy, ng), xsnf(nz, nxy, ng);
    Array<float, 2> psi(nz, nxy);

    Array<float, 3> phif3(nz, nxy, ng), xsnf3(nz, nxy, ng);
    Array<float, 2> psi3(nz, nxy);
    ...
    zfloat3 ji_phif3(nz, nxy, ng);
    zfloat3 ji_xsnf3(nz, nxy, ng);
    zfloat2 ji_psi3(nz, nxy);
    ...
    for (int ig = 0; ig < ng; ++ig) {
        for (int l = 0; l < nxy; ++l) {
            for (int k = 0; k < nz; ++k) {
                //                psi3(l,k) = 0.0;
                sum += (phif3(k, l, ig) op xsnf3(k, l, ig));
    ...
    for (int k = 0; k < nz; ++k) {
        for (int l = 0; l < nxy; ++l) {
            ...
            for (int ig = 0; ig < ng; ++ig) {
                auto                       lk0 = k * nxy + l;
                sum += m_phif(k, l, ig) op m_xsnf(k, l, ig);
    ...
    zfor(k, nz) {
        zfor(l, nxy) {
            ji_psi3(k, l) = 0.0;
            zfor(ig, ng) {
                sum += (ji_phif3(k, l, ig) op ji_xsnf3(k, l, ig));
            }
        }
    }
    end = clock();
    ...
    for (int k = 0; k < nz; ++k) {
        for (int l = 0; l < nxy; ++l) {
            psi(k, l) = 0.0;
            for (int ig = 0; ig < ng; ++ig) {
                sum += (phif(k, l, ig) op xsnf(k, l, ig));
...
//             ffor(k, 1, nz) {
//                 ffor(il, 1, nloop)
// //                multiply(xs(all, all, alk, l),
// //                         flux(all, all, alk, l),
// //                         rate(all, all, alk, l), sum);
//                 multiply(k,xs,
//                          flux,
//                          rate,sum);
// //                ffor(la,0,na-1) {
