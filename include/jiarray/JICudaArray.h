...
public:
    ...
    JIARRAY_HD JICudaArray(std::initializer_list<T> il)
        : JICudaArray(il.size()) {
        std::copy(il.begin(), il.end(), mm);
    }

    JIARRAY_HD JICudaArray(const JICudaArray<T, RANK>& array) {
        nn = array.nn;
        mm = array.mm;
    ...
    JIARRAY_HD virtual ~JICudaArray() {
        destroy();
    }
    ...
    JIARRAY_HD void initByRankSize(int size, const int* rankSizes, const int* offsets, T* memory) {
    ...
    JIARRAY_HD void initByRankSize(int size, const int* rankSizes, const int* offsets) {
        destroy();
        if (size == 0) return;
    ...
    JIARRAY_HD void init(decltype(INTS)... array_sizes, T* memory_) {
        JIARRAY_CHECK_NOT_ALLOCATED();
    ...
    JIARRAY_HD inline JICudaArray<T, RANK - sizeof...(INDEX)> slice(INDEX... index) const {
    ...
    JIARRAY_HD inline T* getMemory(int idx = 0) {
        return mm + idx;
    }

    JIARRAY_HD inline const T* getMemory(int idx = 0) const {
        return mm + idx;
    }
    ...
    JIARRAY_HD inline const T* data() const {
        return mm;
    }
    ...
    JIARRAY_HD inline int getSize() const {
        return nn;
    }

    JIARRAY_HD inline const int* getRankSize() const {
        return rankSize;
    }

    JIARRAY_HD inline const int* getSizeOfRank() const {
        return sizes;
    }
    ...
    JIARRAY_HD inline const int* getOffset() const {
        return offset;
    }

    JIARRAY_HD inline const int& getOffset(int rank) const {
        return offset[rank - JIARRAY_OFFSET];
    }

    JIARRAY_HD inline T& at(decltype(INTS)... index) {
    ...
    JIARRAY_HD inline const T& at(decltype(INTS)... index) const {
    ...
    JIARRAY_HD inline const T& operator()(decltype(INTS)... index) const {
        return at(index...);
    }

    JIARRAY_HD inline T& operator()(decltype(INTS)... index) {
        return at(index...);
    }
    ...
    template <typename... INDEX>
    JIARRAY_HD inline const T* data(INDEX... index) const {
    ...
    JIARRAY_HD inline JICudaArray<T, RANK>& operator=(const std::initializer_list<T>& list) {
    ...
    JIARRAY_HD inline JICudaArray<T, RANK>& operator=(const T& val) {
        assert(nn > 0);
        for (int i = 0; i < nn; ++i) {
            mm[i] = val;
        }
    ...
    JIARRAY_HD inline JICudaArray<T, RANK>& operator=(const T* array) {
        assert(nn > 0);
    ...
    JIARRAY_HD inline JICudaArray<T, RANK>& operator=(const JICudaArray<T, RANK>& array) {
    ...
    JIARRAY_HD inline JICudaArray<T, RANK>& operator=(const JIArray<T, RANK>& array) {
        if (allocated == JIARRAY_ALLOCATED_NONE && mm == nullptr) {
            initByRankSize(array.getSize(), array.getRankSize(), array.getOffset());
        } else {
            JIARRAY_CHECK_SIZE(nn, array.getSize());
    ...
    JIARRAY_HD inline bool operator==(const JICudaArray<T, RANK>& array) const {
        if (nn != array.nn)
            return false;
    ...
    JIARRAY_HD friend inline T dot(const JICudaArray<T, RANK>& array1, const JICudaArray<T, RANK>& array2) {
        // #ifdef JIARRAY_DEBUG
        for (int rank = 0; rank < RANK; ++rank) {
    ...
    JIARRAY_HD inline FastArray<int, RANK> findFirst(const T& item) const {
        int loc = -1;
        T   value;
    ...
    JIARRAY_HD inline const T& last() const {
        assert(nn > 0);
        return mm[nn - 1];
    }

    JIARRAY_HD inline FastArray<int, RANK> maxloc() {
        int maxloc = 0;
        T   maxval = mm[0];
    ...
    JIARRAY_HD inline FastArray<int, RANK> maxloc(int from, int to) {
        int maxloc = 0;
    ...
    JIARRAY_HD inline const T& back() const {
        return mm[nn - 1];
    }
    ...
    struct Iterator {
        ...
        JIARRAY_HD Iterator(pointer ptr) : m_ptr(ptr) {};

        JIARRAY_HD inline Iterator operator+(difference_type n) const {
        ...
        JIARRAY_HD inline Iterator& operator+=(difference_type n) {
            m_ptr += n;
            return *this;
        }
        ...
        JIARRAY_HD inline T& operator[](difference_type n) {
            return *(m_ptr + n);
        }

        JIARRAY_HD inline const T& operator[](difference_type n) const {
            return *(m_ptr + n);
        }

        JIARRAY_HD inline difference_type operator-(const Iterator& other) const {
            return m_ptr - other.m_ptr;
        }

        JIARRAY_HD inline bool operator<(const Iterator& other) const {
            return m_ptr < other.m_ptr;
        }
        JIARRAY_HD inline bool operator<=(const Iterator& other) const {
            return m_ptr <= other.m_ptr;
        ...
        JIARRAY_HD inline reference operator*() {
            return *m_ptr;
        }

        JIARRAY_HD inline reference operator*() const {
            return *m_ptr;
        }

        JIARRAY_HD inline pointer operator->() {
            return m_ptr;
        }

        JIARRAY_HD inline const pointer operator->() const {
            return m_ptr;
        }

        // Prefix increment
        JIARRAY_HD inline Iterator& operator++() {
            m_ptr++;
            return *this;
        }

        // Postfix increment
        JIARRAY_HD inline Iterator operator++(int) {
            Iterator tmp = *this;
            ++(*this);
            return tmp;
        }

        JIARRAY_HD inline const Iterator operator++(int) const {
            Iterator tmp = *this;
            ++(*this);
            return tmp;
        }
        ...
        JIARRAY_HD inline friend bool operator==(const Iterator& a, const Iterator& b) {
...
public:
    JIARRAY_HD inline Iterator begin() {
        return Iterator(mm);
    }
    JIARRAY_HD inline Iterator end() {
        return Iterator(mm + nn);
    } // 200 is out of bounds

    JIARRAY_HD inline const Iterator begin() const {
        return Iterator(mm);
    }
    JIARRAY_HD inline const Iterator end() const {
        return Iterator(mm + nn);
    } // 200 is out of bounds
